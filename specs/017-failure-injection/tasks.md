...
- [ ] **T1 — test first**: `backend/tests/test_failure.py` — a run with `simulate_failure`
...
- [ ] **T2 — implement**: `SimulateFailure` enum + `ChatRequest.simulate_failure` field
...
- [ ] **T3 — test first**: `tool_error` (with a tool-triggering prompt) → an `mcp.call`
...
- [ ] **T4 — implement**: in `tools_node`, when `simulate_failure == "tool_error"`,
...
- [ ] **T5 — test first**: `llm_timeout` → an LLM stage END `data` carries the timeout
  error and the run ends in a clean degraded state (no hang, no unhandled 500); a
  degraded answer is set.
- [ ] **T6 — implement**: in `think_node` (and a guard in `generate_node`), inject +
  catch the simulated `TimeoutError`; record the error on `data`, set the degraded
  answer, route to `respond`.

## Phase 4 — config advertise + frontend control (AC4)

- [ ] **T7 — test first**: `backend/tests/test_config.py` — `/api/config.failure_modes`
  equals the `SimulateFailure` values. `experiment.test.ts` — the control is
  per-conversation and persists until set back to `none`.
- [ ] **T8 — implement**: add `failure_modes` to `/api/config`; surface it in the
  frontend config hook; add the "Simulate failure" selector to `SettingsPanel.tsx`
  (options from `failure_modes`); read `data.error` on the MCP/LLM readout (badged).
  Add `simulate_failure?` to the TS request mirror + optional `SimulatedError` shape.

## Phase 5 — i18n (AC5, §4)

- [ ] **T9 — test first**: parity — selector/option/degraded strings exist in en **and**
  pt.
- [ ] **T10 — implement**: add the strings to `frontend/src/i18n/strings.ts` (en + pt).

## Phase 6 — Verify & refactor

- [ ] **T11 — gates**: `ruff check .` · `ruff format .` · `pytest -q` (with a key) ·
...
- [ ] Omitting `simulate_failure` reproduces today's run byte-for-byte (AC1 guard green)
- [ ] No new `Stage`/`Phase`/`TraceEvent` type; only `ChatRequest` gains the field
- [ ] `/api/config` advertises `failure_modes`; control scoped per conversation
