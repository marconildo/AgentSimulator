...
- [ ] **T1 — test first**: `frontend/src/lib/traceCache.test.ts` — `loadTrace(id)` fetches
  once then serves from cache; a 404 resolves to an `expired` result (no throw).
- [ ] **T2 — implement**: `frontend/src/lib/traceCache.ts` over the existing `fetchTrace`
  (memoize by `trace_id`; `{ ok, events } | { expired: true }`). Shared by 018/020.

## Phase 2 — Static load into the simulator (AC1, AC3)

- [ ] **T3 — test first**: `frontend/src/store/useSimulator.loadTrace.test.ts` —
...
- [ ] **T4 — implement**: add `loadTrace(events)` to `useSimulator` (static settled load,
  no timers; guarded against an active run).

## Phase 3 — Auto-load on switch + click to revisit (AC2)

- [ ] **T5 — test first**: `useChat` — `openSession` auto-loads the latest message's
  trace (canvas not empty); an expired latest trace sets `traceExpired` (click hint);
  `selectMessage(id)` loads that turn.
- [ ] **T6 — implement**: wire `openSession` auto-load + `selectMessage` via the loader →
  `loadTrace`; track `traceExpired`.

## Phase 4 — i18n + UI (AC5)

- [ ] **T7 — test first**: parity — `trace.*` strings exist in en **and** pt.
- [ ] **T8 — implement**: add the strings; in `ChatPanel.tsx`, make past agent messages
...
- [ ] **T10 — gates**: `npm test` (Vitest) · `npm run build` — green. No protocol change;
  consumes existing `TraceSummary`.

## Definition of done

- [ ] Core acceptance criteria map to passing tests (AC1, AC2, AC3, AC5)
- [ ] AC4 (hover emphasis) tracked as secondary, after 014
- [ ] Re-opening a conversation never leaves a dead canvas; expired → clear state
- [ ] Loader memoizes by `trace_id` (the shared 018/020 mechanism)
- [ ] No protocol change; `trace.*` strings exist in en **and** pt
