...
- [x] **T1 — helpers + step logging (AC1)**: create `e2e/helpers.ts` lifting
...
- [x] **T2 — summary reporter (AC2)**: write `e2e/summary-reporter.ts` (Playwright
...
- [x] **T3 — memory test (AC3)**: `e2e/memory.spec.ts` — send a fact-setting turn,
...
- [x] **T4 — drill-in helpers**: add `openStationFullView(page, id)` +
  `expectDrillInHasData(page)` (DetailShell visible + empty placeholder absent) to
  `helpers.ts`.
- [x] **T5 — drill-ins test (AC4)**: `e2e/drilldowns.spec.ts` — `describe.serial`,
...
- [x] **T6 — hop + traces helpers**: add `clickHop(page, src, dst)` and
  `openExecutionTraces(page)` to `helpers.ts`.
- [x] **T7 — inspection test (AC5 + AC6)**: `e2e/inspection.spec.ts` —
  ...
  row (AC6).
- [x] **T8 — protocol mirror**: n/a (no `Stage`/protocol change).
- [x] **T9 — i18n**: n/a (no product prose; test-harness log text only).
- [x] **T10 — cloud map**: n/a (no new tier/station).
