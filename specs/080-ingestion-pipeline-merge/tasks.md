...
- [ ] **T1 — test first (AC1)**: in `backend/tests/test_ingestion.py`, assert a PDF
...
- [ ] **T2 — protocol (schemas)**: add `RAG_INGEST_TOKENIZE` + `RAG_INGEST_METADATA`
  to `Stage` in `backend/app/schemas.py` with the ingestion comment block.
- [ ] **T3 — implement (AC1)**: rework `ingest_pdf` in `rag/ingestion.py` into the
  six-stage sequence; make T1 green. Apply the same two new stages to
  `reingest_corpus` in `rag/ingest.py`.
- [ ] **T4 — test first (AC2)**: assert `rag.ingest.tokenize` END carries a
  per-chunk token list of length == num_chunks and a `total_tokens` metric == sum.
- [ ] **T5 — implement (AC2)**: move token counting into the tokenize stage; green.
- [ ] **T6 — test first (AC3)**: assert `rag.ingest.metadata` END list length ==
  num_chunks **and** the metadata persisted on the stored Chroma chunks matches.
- [ ] **T7 — implement (AC3)**: build metadata in the metadata stage, hand the same
  records to the store step; green.
- [ ] **T8 — test (AC4)**: confirm a no-upload chat fires none of
  `storage.upload`/`rag.ingest.*` (extend the existing agent guard test).
- [ ] **T8a — test first (AC10)**: in `test_ingestion.py`, assert an upload chunks
...
- [ ] **T8b — implement (AC10)**: swap `chunk_text` → `chunk_texts(text,
  active_chunk_strategy())` in `ingest_pdf`; surface `strategy` in the chunk END;
  green.

## Protocol mirror + visual model merge

- [ ] **T9 — protocol mirror (AC5)**: add both new members to the `Stage` union in
  `frontend/src/types/events.ts`.
- [ ] **T10 — test first (AC6/AC8)**: update/extend `lib/storage-stages.test.ts` +
...
- [ ] **T11 — implement (AC5/AC6/AC8)**: in `stations.ts` remove the `storage`
...
- [ ] **T15 — i18n (AC9)**: add all new phase labels / drill-in headings / glossary
  entries in en **and** pt (table in `plan.md`).
- [ ] **T16 — cloud map**: confirm the Object-store phase carries Blob/S3/Cloud
  Storage names (no station lost a cloud example).
- [ ] **T17 — refactor**: clean up, fix any count-based legacy assertions to be
  structural, keep everything green.
- [ ] **T18 — demo flag**: per the standing GitHub Pages directive, flag whether
