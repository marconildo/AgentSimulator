...
- [ ] **T1 — test first**: in `backend/tests/test_db.py`, add
  ...
  (AC2)
- [ ] **T2 — implement T1**: in `backend/app/db/store.py`:
...
- [ ] **T10 — implement T9**: in `backend/app/schemas.py`, add
  ...
  the `db.write` span.
- [ ] **T11 — test first**: add `test_list_messages_returns_documents_field`
  — pre-existing message → `documents: []`; attached message → populated. (AC6)
- [ ] **T12 — verify**: covered by T2; just ensure the REST shape (no Pydantic
  re-wrap) carries the list through. Add the explicit assertion on the GET.

### Frontend — pending attachments + send snapshot (AC1, AC9)

- [ ] **T13 — test first**: in `frontend/src/store/useChat.test.ts`, extend
...
- [ ] **T14 — implement T13**: in `frontend/src/store/useChat.ts`:
...
- [ ] **T15 — test first**: add `test "upload during in-flight send queues
...
- [ ] **T16 — implement T15**: confirmed by T14's snapshot-then-clear order.
  Adjust the streamChat/batchChat shims in `lib/sse.ts` to thread the new
  option through the JSON body (one param each).
- [ ] **T17 — implement (transport)**: update `frontend/src/lib/chatApi.ts`
...
- [ ] **T19 — implement T18**: in `frontend/src/components/ChatPanel.tsx`:
...
- [ ] **T25 — manual verify**: launch the app, upload a PDF, send a message,
  ...
  chip queues for the next turn.
- [ ] **T26 — spec status & memory**: flip `spec.md` Status from `clarified` →
...
- [ ] No protocol drift: `schemas.py` ↔ `events.ts` unchanged for `TraceEvent`
