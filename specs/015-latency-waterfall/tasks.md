...
- [ ] **T1 — test first**: in `frontend/src/lib/waterfall.test.ts`, build a linear log
...
- [ ] **T2 — test first**: a log with a ReAct loop (reason → tools → reason → tools)
  yields **two** `reason` and **two** `tools` segments, order preserved (AC2).
- [ ] **T3 — test first**: total == `toMs(last.ts) − toMs(first.ts)` within rounding;
...
- [ ] **T4 — implement**: `frontend/src/lib/waterfall.ts` — `WaterfallSegment` +
...
- [ ] **T5 — test first**: assert `timeline.timing.{title,total,overhead,empty}` exist
  in **both** en and pt (parity).
- [ ] **T6 — implement**: add those strings to `frontend/src/i18n/strings.ts` (en + pt).

## Phase 3 — Timing panel (UI)

- [ ] **T7 — implement**: `frontend/src/components/TimingPanel.tsx` — proportional bars
...
- [ ] **T8 — implement**: mount `TimingPanel` in `frontend/src/components/Timeline.tsx`
  (collapsible, beside/under the phase rail); empty-state copy when no run.

## Phase 4 — Verify & refactor

- [ ] **T9 — gates**: `npm test` (Vitest) green · `npm run build` (`tsc --noEmit` +
...
- [ ] No protocol change; only *reads* `ts` / `latency_ms` / `STAGE_TO_PHASE`
- [ ] All new chrome exists in en **and** pt; phase names reuse `phaseLabelsFor`
