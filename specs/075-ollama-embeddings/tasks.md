...
- [ ] **T1 — test (red)**: `test_ollama_embeddings.py` — `get_embeddings()` with the
...
- [ ] **T2 — impl (green)**: `config.py` `embedding_provider` (env `EMBEDDING_PROVIDER`);
...
- [ ] **T3 — test (red)**: `GET/PUT /api/settings/embeddings` round-trips provider+model,
  persists across a new store, defaults from env. (AC2)
- [ ] **T4 — impl (green)**: `GET/PUT /api/settings/embeddings` in `main.py`
  (`app_config` keys `embedding_provider`/`embedding_model`); fold effective values into
  `/api/config`.
- [ ] **T5 — test (red)**: `index_matches_model()` returns False when the persisted
...
- [ ] **T6 — impl (green)**: stamp `{embedding_provider, embedding_model}` into the Chroma
  collection metadata at build; compare it in `index_matches_model()` alongside the dim.
- [ ] **T7 — test (red)**: provider=ollama + unreachable server → `ConversationStore` /
  app startup boots, index build is caught (no crash). (AC5)
- [ ] **T8 — impl (green)**: confirm/guard the lifespan + store paths swallow an
  unreachable-embedding error (reuse existing try/except; add coverage if a path is exposed).

## Backend — integration (marker)

- [ ] **T9 — integration**: `test_ollama_embeddings.py::test_openai_free_rag`
...
- [ ] **T10 — test (red)**: `EmbeddingsSection.test.tsx` — selecting Ollama persists
...
- [ ] **T11 — impl (green)**: `chatApi.ts` (`get/setEmbeddingSettings`, `AppConfig`
  embedding fields); `EmbeddingsSection` mounted in `SettingsPage`; rebuild reuses the 072
  re-ingest stream.
- [ ] **T12 — i18n**: add all `settings.embeddings.*` strings in **en + pt**. (AC7)

## Close-out

- [ ] **T13 — regression**: full backend `pytest -q` + FE `npm test` + `npm run build`
  green; default OpenAI-embeddings path unchanged. (AC8)
- [ ] **T14 — refactor**: tidy, keep green; flip `spec.md` status `planned → done`.
- [ ] **T15 — demo check** (standing rule): the 058 GitHub Pages build runs no provider —
  embeddings change shouldn't need a re-capture; confirm + note.

## Definition of done

- [ ] Every acceptance criterion maps to a passing test (AC4 marker-gated)
- [ ] `ruff check .` + `ruff format .` clean
- [ ] `pytest -q` green with `OPENAI_API_KEY` (keyless + Ollama-keyless tests pass w/o key)
- [ ] `npm run build` (`tsc --noEmit` + build) + `npm test` green
- [ ] No protocol change (no Stage/station/phase touched)
- [ ] New user-facing text exists in en **and** pt
- [ ] An Ollama-only deployment (chat + embeddings on Ollama) runs with **no OpenAI key**
