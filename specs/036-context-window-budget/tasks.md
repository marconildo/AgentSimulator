...
- [ ] **T1 — test (AC1)**: `backend/tests/test_context_budget.py` — `context_window()` returns the
  real size for `gpt-4o-mini`/`gpt-4o`/`gpt-4.1*` and `DEFAULT_CONTEXT_WINDOW` (non-zero) for an
  unknown model. *(red)*
- [ ] **T2 — impl (AC1)**: `backend/app/llm/context.py` — `MODEL_CONTEXT_WINDOW`, `DEFAULT_CONTEXT_WINDOW`,
  `context_window(model)`. Module docstring flags it a labelled teaching approximation. *(green)*
- [ ] **T3 — test (AC2, AC3)**: in the same test file — `context_budget(...)` returns the six used
...
- [ ] **T4 — impl (AC2, AC3)**: implement `context_budget(...)` in `context.py` (reuse the
  `rag/ingestion.py` encoder). *(green)*
- [ ] **T5 — test (AC4)**: extend `backend/tests/test_agent.py` — a real run's last `llm.prompt` END
...
- [ ] **T6 — impl (AC4)**: in `backend/app/agent/graph.py` `think_node`, attach `context_window`
  + `context_budget` to `prompt_rec.data` next to `prompt_preview`/`context`. *(green)*

## Protocol mirror

- [ ] **T7 — protocol mirror (AC4)**: `frontend/src/types/events.ts` — add `ContextBudget` type +
...
- [ ] **T8 — test (AC5, AC6, AC9)**: `frontend/src/lib/contextBudget.test.ts` — `deriveBudget`:
  ...
  `estimated` flag. *(red)*
- [ ] **T9 — impl (AC5, AC6, AC9)**: `frontend/src/lib/contextBudget.ts` — `deriveBudget(events, cursor)`. *(green)*
- [ ] **T10 — test (AC7)**: in `contextBudget.test.ts` — `gridCells(...)` allocates colored cells in
  category order with the remainder as Free space; sums to `cellCount`; legend includes only non-zero
  categories + Free space. *(red)*
- [ ] **T11 — impl (AC7)**: `gridCells(...)` in `contextBudget.ts`; render the grid + "used / max (pct)"
  headline (model name) + legend in `AgentDetail.tsx`, replacing the composition bar. *(green)*
- [ ] **T12 — test (AC8)**: update `frontend/src/lib/turnDiff.test.ts` — `contextSections` prefers the
  emitted `context_budget`, falls back to `chars/4`; new `Section` key set; diff parity holds. *(red)*
- [ ] **T13 — impl (AC8)**: update `turnDiff.ts` (`Section` keys + `contextSections` source); rewire
  `TurnCompare` labels in `AgentDetail.tsx`. *(green)*

## i18n + polish

- [ ] **T14 — i18n (AC10)**: add the new `agentDetail.*` strings (plan table) in **en + pt**; confirm
  the parity test passes.
- [ ] **T15 — refactor**: dedupe `lastEnd`/section mapping shared by `turnDiff` and `contextBudget`;
...
- [ ] `pytest -q` green (with `OPENAI_API_KEY`; keyless budget tests pass without one)
- [ ] `npm run build` passes (`tsc --noEmit` + build) · `npm test` (Vitest) green
- [ ] Protocol mirror in sync (`schemas.py` ↔ `events.ts`); no new `Stage`; `STAGE_TO_STATION`
      & `STAGE_TO_PHASE` still total
- [ ] All new user-facing text exists in en **and** pt (§4); cloud map n/a (§5)
- [ ] `spec.md` status updated to `in-progress` → `done`
