...
- [ ] **T1 — test first**: `frontend/src/lib/citations.test.ts` — a sentence that shares
  a ≥N-word significant n-gram with a source gets a citation to that source (AC1);
  ties resolve to the longest shared n-gram.
- [ ] **T2 — test first (negative)**: a sentence with no qualifying overlap gets **no**
  citation — proving no fabrication (AC2).
- [ ] **T3 — test first**: each emitted citation carries the hover payload (source
  kind/id, tool args or chunk score, snippet) (AC3).
- [ ] **T4 — implement**: `frontend/src/lib/citations.ts` — `Citation`/`CitationSource`,
...
- [ ] **T5 — test first**: parity — `citation.*` chrome exists in en **and** pt.
...
- [ ] **T7 — implement**: in `frontend/src/components/AgentDetail.tsx`, render the answer
...
- [ ] **T8 — gates**: `npm test` (Vitest) · `npm run build` — green. Function stays pure;
  protocol untouched.

## Definition of done

- [ ] Every acceptance criterion in `spec.md` maps to a passing test (AC1–AC4)
- [ ] Negative test proves no fabricated citations (honest by construction)
- [ ] No protocol change; reads existing `mcp.call` / `rag.retrieve` data only
- [ ] Citation chrome exists in en **and** pt; args/snippets verbatim
