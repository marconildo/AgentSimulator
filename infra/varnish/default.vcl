...
sub vcl_deliver {
    # 093-waf-block-visualization: ensure the cross-origin browser can READ error
    # responses (esp. the WAF's 403, generated by ModSecurity, which carries no CORS
    # headers) — otherwise fetch() rejects with a CORS error and the FE can't tell a
    # block from a network failure. Only add it when absent, so normal 200s keep the
    # backend's own CORS header (no duplicate header). No credentials are sent, so
    # "*" is safe.
